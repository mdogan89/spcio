...
//            _color = PlayerManager.Instance.skinColor; // Get the player's color from PlayerManager
...
//                GetComponent<MeshRenderer>().materials[0].SetColor("_EmissionColor", _color); // Set the player's color if the skin ID is 3 (custom skin)
...
//        if (scoreText != null&&SceneManager.GetActiveScene().buildIndex != 2)
//        {
...
//        if (bot == null) {
//            if (PlayerManager.Instance.skinId == 3)
//                _animator.SetTrigger("EmissiveEat"); // Trigger custom eating animation if using custom skin
//            else
...
//        }
//        _Size += other.Size; // Example of increasing size
...
//        SceneManager.LoadScene(0); // Load the title scene when the menu button is clicked
//    }
//}
